...
{
    public partial class RmProductionWeightScaleBox : System.Web.UI.Page
    {
        #region Page Load
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session[WebUtility.SessionCurrentUserObj] != null)
            {
                if (!IsPostBack)
                {
                    txtLastTrId.Text = new RmilBoxMasterDa(true).LastTransactionId().ToString();
                   LoadProduct();
                }
                ...
                try
                {
                    using (var scope = new TransactionScope(TransactionScopeOption.Required))
                    {
                        using (RMILCSDbEntities db = new RMILCSDbEntities())
                        {
                            ...
                            RmilBoxMaster rmilProdMaster = new RmilBoxMaster();

                            Random numberGenerator = new Random();
                            int ranNumber = numberGenerator.Next(1000000000, 2147483647);

                            var ranUniqueNo = "MC"+rmilProdMaster.RmPmId +
                            ...
                            //db.SaveChanges();
                            new RmilBoxMasterDa(false).Insert(rmilProdMaster);

                            // Insert into RmilProdDetails
                            List<RmilBoxDetails> prodDetailsList = new List<RmilBoxDetails>();
                            ...
                            //for (int i = 1; i <= qty; i++)
                            //{
                            //    MyNumber = dataGenerator.Next(10000000, 99999999);
                            //    if (!randomList.Contains(MyNumber))
                            //        randomList.Add(MyNumber);
                            //}
                            for (int i = 1; i <= qty; i++)
                            {
                                MyNumber = dataGenerator.Next(1000, 9999);
                            ...
                            for (int i = 0; i < qty; i++)
                            {
                                RmilBoxDetails d = new RmilBoxDetails();
                                d.RmPdId = 0;
                                d.RmPmId = new RmilBoxMasterDa(true).GetLastRecord().RmPmId;
                                ...
                                if (rmilProdMaster.PDate != null)
                                {
                                    ...
                                    d.DBarcode = "MC"+dt.Year.ToString().Substring(2) + "" + dt.Month.ToString().PadLeft(2, '0') + "" +
        ...
        {
            var mTrId = txtLastTrId.Text;
            Response.Redirect("~/Reports/RptBoxBarcode.aspx?pmId=" + mTrId);
        }
        #endregion
    }
}